...
namespace FastReflectionLib
{
    ...
    public class FieldAccessor : IFieldAccessor
    {
        ...
        private void InitializeSet(FieldInfo fieldInfo)
        {
            // target: (((TInstance)instance).Field) = value;
            ...
            // (object)(((TInstance)instance).Property)
            var castFieldValue = Expression.Assign(fieldAccess, Expression.Convert(value, fieldInfo.FieldType));

            // Lambda expression
            var lambda = Expression.Lambda<Action<object, object>>(castFieldValue, instance, value);
